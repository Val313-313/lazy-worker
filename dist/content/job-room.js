(()=>{(function(){console.log("[Lazy Worker] job-room.ch content script loaded"),setTimeout(S,2e3);function S(){v();let e=location.href;new MutationObserver(()=>{location.href!==e&&(e=location.href,console.log("[Lazy Worker] SPA navigation detected:",e),document.getElementById("lw-float-btn")||v())}).observe(document.body,{childList:!0,subtree:!0})}function v(){const e=document.getElementById("lw-float-btn");e&&e.remove();const n=document.createElement("div");n.id="lw-float-btn",n.innerHTML=`
      <style>
        #lw-float-btn {
          position: fixed;
          bottom: 24px;
    ...
    `,document.body.appendChild(n),document.getElementById("lw-trigger").addEventListener("click",q),document.getElementById("lw-panel-close").addEventListener("click",L)}function q(){const e=document.getElementById("lw-panel");e.classList.toggle("active"),e.classList.contains("active")&&A()}function L(){document.getElementById("lw-panel").classList.remove("active")}async function A(){const e=document.getElementById("lw-panel-content");try{if(!chrome?.storage?.local)throw new Error("Chrome storage not available");console.log("[Lazy Worker] Loading applications from storage...");const n=await chrome.storage.local.get("applications");console.log("[Lazy Worker] Storage result:",n);const l=n.applications||[];if(l.length===0){e.innerHTML='<div class="lw-empty">Keine Bewerbungen gespeichert.<br>Erfasse zuerst Jobs!</div>';return}l.sort((a,o)=>new Date(o.appliedAt)-new Date(a.appliedAt)),e.innerHTML=l.map(a=>`
